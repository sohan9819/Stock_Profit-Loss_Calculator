...
form.addEventListener('submit' , function(e){
    ...
    if ( Profit > 0){
        ...
        result.innerText = `You have made a profit of ${Profit*parseInt(quantity)} that is ${profitPrecentage(Profit , costPrice)}%`
    } else if ( Loss > 0){
        console.log(lossPercentage(Loss , costPrice))
        result.innerText = `You have made a loss of ${Loss*parseInt(quantity)} that is ${lossPercentage(Loss , costPrice)}%`
    }
})
